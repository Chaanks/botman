...
from tasks import HelloTask, GatherTask
...
class BotManagerUI(App):
    ...
    def on_input_submitted(self, event: Input.Submitted):
        ...
        if command.startswith("hello"):
        ...
        elif command.startswith("gather"):
            # gather <resource> <amount> [bot]
            parts = command.split()
            if len(parts) < 3:
                self._add_log("Usage: gather <resource> <amount> [bot]")
                return

            resource = parts[1]
            amount = int(parts[2])
            target_bot = parts[3] if len(parts) > 3 else self.bots[0]
            ...
            self._add_log(f"Sent gather task ({resource} x{amount}) to {target_bot}")

    ...
    if task_type == "gather":
        return Div(
            Div(
                ...
                P(
                    "The code of the resource to gather",
        ...
        return Div(
            ...
            Div(
                ...
                P(
                    "How many times to fight the monster",
                    cls="text-xs text-gray-500 mt-1",
                ),
            ),
            cls="space-y-4",
        ...
        return Div(
            ...
            Div(
                Label(
                    Input(type="checkbox", name="recycle", value="true", cls="mr-2"),
                    "Recycle after crafting",
                    cls="text-sm text-gray-400 flex items-center cursor-pointer",
                ),
                P(
                    "Automatically recycle crafted items for materials",
                    cls="text-xs text-gray-500 mt-1 ml-6",
                ),
            ),
            cls="space-y-4",
        )
...
def BotCard(bot_name: str, bot_state: dict, map_tile=None):
    ...
    autonomous_mode = bot_state.get("autonomous_mode", True)
            ...
            Div(
                ...
                Div(
                    ...
                    Button(
                        Span(
                            "Auto: ",
                            Span("ON" if autonomous_mode else "OFF",
                                 cls=f"font-bold {'text-green-400' if autonomous_mode else 'text-red-400'}"),
                            cls="text-xs",
                        ),
                        hx_post="/bot_toggle_autonomous",
                        hx_vals=f'{{"bot_name": "{bot_name}", "enabled": "{"false" if autonomous_mode else "true"}"}}',
                        hx_swap="none",
                        cls=f"ml-auto px-2 py-1 rounded text-xs transition-colors {'bg-green-900/30 border border-green-600 hover:bg-green-900/50' if autonomous_mode else 'bg-gray-700 border border-gray-600 hover:bg-gray-600'}",
                        title="Toggle autonomous behavior when idle",
                    ),
                ...
                Div(
                    ...
                    Div(f"Account: {account}", cls="text-sm text-gray-400 mt-2 mb-1")
                    if character
                    else None,
                    Div(f"Level {level}", cls="text-sm text-gray-400 mb-2")
                    if character
                    else None,
                    Div(
                ...
                Div(
                    ...
                    P(f"{queue_size} queued", cls="text-xs text-gray-500")
                    if queue_size > 0
                    else None,
                    cls="mt-4 space-y-1",
                ),
...
def DashboardPage(state: dict, world=None):
    ...
    return Container(
        ...
        Div(
            ...
            Div(
                A(
                    UkIcon("trophy", height=18, cls="mr-2"),
                    "Achievements",
                    href="/achievements",
                    cls="inline-flex items-center px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium rounded-md transition-colors",
                ),
                A(
            ...
            Div(
                ...
                Div(
                    ...
                    Div(
                        ...
                        ProgressBar(
                            "XP", character.xp, character.max_xp, "bg-green-500"
                        ),
                        cls="space-y-3 mb-4",
                    ),
                    # Quick Stats Grid
                ...
                Div(
                    ...
                    Grid(
                        Div(
                            ...
                            P(
                                str(character.stats.total_attack()),
                                cls="text-white font-medium text-xl",
                            ),
                        ),
                        Div(
                            Div(
                            ...
                            P(
                                str(character.stats.total_damage()),
                                cls="text-white font-medium text-xl",
                            ),
                        ),
                        Div(
                            ...
                            P(
                                str(character.stats.total_resistance()),
                                cls="text-white font-medium text-xl",
                            ),
                        ),
                        cols=3,
...
def AchievementCard(achievement, is_selected: bool = False):
    """Achievement card with progress bar and selection checkbox"""
    # Calculate progress percentage
    current = achievement.get("current", 0)
    ...
    if is_completed:
        ...
        bg_color = "bg-green-900/10"
    elif is_selected:
        border_color = "border-blue-500"
        bg_color = "bg-blue-900/10"
    else:
        border_color = "border-gray-700"
        bg_color = "bg-gray-800/50"
    ...
    return Card(
        Div(
            ...
            Div(
                Div(
                    Input(
                        type="checkbox",
                        name="achievement_codes",
                        value=achievement["code"],
                        checked=is_selected,
                        disabled=is_completed,
                        cls="w-4 h-4 rounded border-gray-600 bg-gray-700 text-blue-600 focus:ring-2 focus:ring-blue-500",
                        hx_post="/achievements/toggle",
                        hx_vals=f'{{"code": "{achievement["code"]}"}}',
                        hx_target="#main-content",
                        hx_swap="outerHTML",
                    ),
                    H3(
                        achievement["name"], cls="text-lg font-semibold text-white ml-3"
                    ),
                    cls="flex items-center",
                ),
                Span(
                    f"+{achievement['points']} pts",
                    cls="text-xs font-medium text-yellow-400 bg-yellow-900/20 px-2 py-1 rounded",
                ),
                cls="flex items-center justify-between mb-3",
            ),
            # Description
            P(achievement["description"], cls="text-sm text-gray-400 mb-3"),
            # Type badge
            Div(
                Span(
                    achievement["type"].replace("_", " ").title(),
                    cls="text-xs text-gray-400 bg-gray-700 px-2 py-1 rounded",
                ),
                cls="mb-3",
            ),
            # Progress bar
            Div(
                Div(
                    Span("Progress", cls="text-xs text-gray-400"),
                    Span(
                        f"{current}/{total}", cls="text-xs text-gray-300 font-medium"
                    ),
                    cls="flex justify-between mb-1",
                ),
                Div(
                    Div(
                        style=f"width: {percentage}%",
                        cls=f"{'bg-green-500' if is_completed else 'bg-blue-500'} h-full rounded-full transition-all duration-300",
                    ),
                    cls="w-full bg-gray-700 rounded-full h-2",
                ),
                cls="mb-2",
            ...
            # Completed badge
            Div(
                UkIcon("check-circle", height=16, cls="text-green-400 mr-2"),
                Span(
                    f"Completed on {completed_at.strftime('%Y-%m-%d')}"
                    if isinstance(completed_at, datetime)
                    else f"Completed on {completed_at}",
        ...
        cls=f"{bg_color} border-2 {border_color} transition-colors",
    )


def AchievementsPage(achievements: list, selected_codes: set, all_achievements: list):
    """Achievements page with filtering and selection"""
    # Group ALL achievements by type for filter buttons
    types = {}
    for ach in all_achievements:
        ach_type = ach.get("type", "other")
        if ach_type not in types:
            types[ach_type] = []
        types[ach_type].append(ach)

    # Count stats from ALL achievements
    total = len(all_achievements)
    completed = sum(1 for ach in all_achievements if ach.get("completed_at"))
    in_progress = len(selected_codes)
    total_points = sum(ach["points"] for ach in all_achievements)
    earned_points = sum(
        ach["points"] for ach in all_achievements if ach.get("completed_at")
    )

    # Filter selected achievements
    selected_achievements = [ach for ach in all_achievements if ach["code"] in selected_codes]
    ...
    return Container(
        ...
        Div(
            Div(
                A(
                    ...
                    hx_target="#main-content",
                    hx_swap="outerHTML",
                    cls="inline-flex items-center hover:text-gray-300 transition-colors mb-4",
                ),
                H1("Achievements", cls="text-3xl font-bold text-white mb-2"),
                P(
                    "Track and select achievements to work towards with your bots",
                    cls="text-gray-400",
                ),
                cls="mb-6",
            ),
            ...
            Grid(
                Card(
                    Div(
                        Div(
                            UkIcon("trophy", height=24, cls="text-yellow-400 mb-2"),
                            P("Total Achievements", cls="text-xs text-gray-400"),
                            P(
                                f"{completed}/{total}",
                                cls="text-2xl font-bold text-white mt-1",
                            ),
                            cls="text-center",
                        ),
                        cls="p-4",
                ...
                Card(
                    Div(
                        Div(
                            UkIcon("star", height=24, cls="text-yellow-400 mb-2"),
                            P("Achievement Points", cls="text-xs text-gray-400"),
                            P(
                                f"{earned_points}/{total_points}",
                                cls="text-2xl font-bold text-white mt-1",
                            ),
                            cls="text-center",
                        ),
                ...
                Card(
                    Div(
                        Div(
                            UkIcon("target", height=24, cls="text-blue-400 mb-2"),
                            P("Active Goals", cls="text-xs text-gray-400"),
                            P(
                                str(in_progress),
                                cls="text-2xl font-bold text-white mt-1",
                            ),
        ...
        Div(
            H2("Active Goals", cls="text-xl font-semibold text-white mb-4"),
            Div(
                Div(
                    *[
                        AchievementCard(ach, True)
                        for ach in selected_achievements
                    ] if selected_achievements else [
                        Div(
                            P("No active goals selected", cls="text-gray-500 text-center py-8"),
                            cls="col-span-full",
                        )
                    ],
                    id="selected-achievements-container",
                    cls="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4",
                ),
                cls="max-h-96 overflow-y-auto",
            ),
            cls="mb-8",
        ) if selected_achievements else None,
        # Filter tabs
        Div(
            H2("All Achievements", cls="text-xl font-semibold text-white mb-4"),
            Div(
                Button(
                    "All",
                    cls="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md",
                    hx_get="/achievements/filter?type=all",
                    hx_target="#achievements-grid",
                    hx_swap="innerHTML",
                ),
                *[
                    Button(
                        ach_type.replace("_", " ").title(),
                        cls="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white text-sm font-medium rounded-md transition-colors",
                        hx_get=f"/achievements/filter?type={ach_type}",
                        hx_target="#achievements-grid",
                        hx_swap="innerHTML",
                    )
                    for ach_type in sorted(types.keys())
                ],
        ...
        Div(
            Div(
                Grid(
                    *[
                        AchievementCard(ach, ach["code"] in selected_codes)
